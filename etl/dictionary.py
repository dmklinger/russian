...
class Forms:
	...
	def add_forms(self, forms):
		...
		# remove empty forms & duplicates
		for form_id in list(self.forms):
			form_list = self.forms[form_id]
			new_form_list = {None if form == '' else form.replace('*', ''): None for form in form_list}
			if None in new_form_list:
				del new_form_list[None]
			self.forms[form_id] = [x for x in new_form_list]
			if len(self.forms[form_id]) == 0:
...
class Dictionary:
	...
	def add_frequencies(self):
		...
		for _, word in self.dict.items():
			collate_word = word.get_word_no_accent().replace('ё', 'е')
			if collate_word in frequencies:
				word.add_frequencies(frequencies[collate_word])
			else:
